...
public class BiTree {
	...
	/*public BiTree(String postOrder, String inOrder, int postIndex, int inIndex,
	int count) {
if (count > 0) {// 先根和中根非空
	char r = postOrder.charAt(postIndex);// 取先根字符串中的第一个元素作为根结点
	int i = 0;
	for (; i < count; i++)
		// 寻找根结点在中根遍历字符串中的索引
	...
	root = new BiTreeNode(r);// 建立树的根结点
	root.setRchild(new BiTree(postOrder,inOrder, postIndex-1,
			inIndex,i ).root);// 建立树的右子树
	root.setLchild(new BiTree(postOrder,inOrder,postIndex-i -1, inIndex-i-2,
			count - i - 1).root);// 建立树的左子树
}
}
*/
	// 中根遍历二叉树基本操作的非递归算法
