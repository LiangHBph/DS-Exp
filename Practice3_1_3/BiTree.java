...
public class BiTree {
	 ...
	 * count) { if (count > 0) {// 先根和中根非空 char r = preOrder.charAt(preIndex);//
	 * 取先根字符串中的第一个元素作为根结点 int i = 0; for (; i < count; i++) // 寻找根结点在中根遍历字符串中的索引 if
	 * (r == inOrder.charAt(i + inIndex)) break;
	 * 
	 * root = new BiTreeNode(r);// 建立树的根结点 root.setLchild(new BiTree(preOrder,
	 * inOrder, preIndex + 1, inIndex, i).root);// 建立树的左子树 root.setRchild(new
...
//后根和中根建树  依据提供先根建树修改  改动之处均加了注释
	public BiTree(String postOrder, String inOrder, int postIndex, int inIndex, int count) {
		if (count > 0) {
			char r = postOrder.charAt(postIndex);
			int i = 0;
			for (; i < count; i++)
				if (r == inOrder.charAt(inIndex - i))//有改动
					break;
			root = new BiTreeNode(r);
			root.setRchild(new BiTree(postOrder, inOrder, postIndex - 1, inIndex, i).root);// 建立树的右子树，有改
			root.setLchild(new BiTree(postOrder, inOrder, postIndex - i - 1, inIndex - i - 1, count - i - 1).root);// 建立树的左子树
		}
	}
